  ...
  return (
    ...
    >
    {
      ...
      <Space size="large" style={{ width: "100%" }} direction="vertical">
        ...
        <div />
        {
          certificateTemplate.info.map((row, index) => (
            <Row key={`info-${index}-${row.label}`}>
              <Col span={8}>
                <div style={{fontSize: 20}}><strong>{row.label}:</strong></div>
              </Col>
              <Col span={16}>
        ...
        <div />
        {
          certificateTemplate.footer.map((row, index) => (
            <Row key={`footer-${index}-${row.label}`} gutter={8}>
              <Col style={{ textAlign: "right" }} offset={8} span={8}>
                <div style={{fontSize: 20}}><strong>{row.label}:</strong></div>
              </Col>
              <Col style={{ textAlign: "right" }} span={8}>
...
const mapStateToProps = (state) => {
  return {
    data: state.data,
    certificateTemplate: state.metadata.certificateTemplate,
    customCertificate: state.metadata.customCertificate,
    formMapping: state.metadata.formMapping
  };
};
